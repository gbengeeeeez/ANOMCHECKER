...
# All UI components. System runs K-Means only.
# =============================================================
...
def hero_section():
    ...
    return html.Div(style={
        ...
        html.H1("Detect Threats with K-Means Clustering",
            style={"color": "white", "fontWeight": "800",
                   "fontSize": "32px", "marginBottom": "10px"}),
        html.P(
            "Upload your network log CSV, define your row range, and let ANOMCHECKER "
            "automatically run K-Means clustering and surface the threats.",
            style={"color": COLORS["subtext"], "fontSize": "15px",
                   "maxWidth": "700px", "marginBottom": "28px"}),
        dbc.Row([
            dbc.Col(html.Div([
                html.Div(s[0], style={"color": COLORS["cyan"],
...
def sidebar():
    return html.Div(style={
        ...
        _card([
            ...
            html.P("Enter which rows to analyze. Max range is 10,000 rows.",
                style={"color": COLORS["subtext"], "fontSize": "12px", "marginBottom": "14px"}),
            dbc.Row([
                dbc.Col([
                    html.Label("Start Row",
                        style={"color": COLORS["subtext"], "fontSize": "12px"}),
                    dbc.Input(id="start-row-input", type="number", min=1, step=1,
                        placeholder="e.g. 1", value=1, debounce=True,
                ...
                dbc.Col([
                    html.Label("End Row",
                        style={"color": COLORS["subtext"], "fontSize": "12px"}),
                    dbc.Input(id="end-row-input", type="number", min=1, step=1,
                        placeholder="e.g. 10000", value=10000, debounce=True,
                        style={"background": COLORS["bg"], "color": "white",
                               "border": "1px solid rgba(0,212,255,0.3)",
                               "borderRadius": "8px"}),
                ], width=6),
            ...
            html.Div(style={"marginTop": "12px"}, children=[
                html.P("Quick presets:",
                    style={"color": COLORS["subtext"], "fontSize": "11px",
                           "marginBottom": "6px"}),
                html.Div(style={"display": "flex", "gap": "6px", "flexWrap": "wrap"},
                    children=[
                        dbc.Button("1K",  id="preset-1k",  size="sm",
                                   outline=True, color="info",    n_clicks=0),
                        dbc.Button("5K",  id="preset-5k",  size="sm",
                                   outline=True, color="info",    n_clicks=0),
                        dbc.Button("10K", id="preset-10k", size="sm",
                                   outline=True, color="success", n_clicks=0),
                    ]),
                html.P("10K = recommended for best accuracy",
        ...
        dbc.Button(
            [dbc.Spinner(size="sm", spinner_style={"display": "none"}),
             " 🚀 RUN ANOMCHECKER"],
            id="run-btn", className="run-btn", color="primary",
            style={"width": "100%", "padding": "14px"}, n_clicks=0,
        ),
...
def welcome_screen():
    steps = [
        ...
        ("3", "View Results",  "Explore K-Means clustering results, metrics, and download flagged threats."),
    ]
    return html.Div(style={"textAlign": "center", "padding": "60px 40px"}, children=[
        html.Div("🛡️", style={"fontSize": "72px", "marginBottom": "20px"}),
        html.H3("Welcome to ANOMCHECKER",
            style={"color": "white", "fontWeight": "800"}),
        html.P("An intelligent K-Means clustering system for cybersecurity threat detection in network logs.",
...
def metric_card(label, value, color, icon):
    return html.Div(className="metric-card",
        style={"borderTop": f"3px solid {color}"}, children=[
            html.Div(icon, style={"fontSize": "24px", "marginBottom": "6px"}),
            html.Div(f"{value}%",
                style={"color": color, "fontSize": "28px", "fontWeight": "800"}),
            html.Div(label,
...
def results_panel(
    ...
    tp = km_metrics["tp"]; tn = km_metrics["tn"]
    fp = km_metrics["fp"]; fn = km_metrics["fn"]
    ...
    return html.Div(style={"padding": "24px"}, children=[
        ...
        html.Div(style={"marginBottom": "20px"}, children=[
            ...
            html.P(
                ...
                f"Algorithm: K-Means  •  "
                f"Samples: {total:,}  •  Threats Flagged: {attacks:,}",
                style={"color": COLORS["subtext"], "fontSize": "13px"}),
        ]),

        # Metric cards
        dbc.Row([
            dbc.Col(metric_card("Accuracy",            km_metrics["accuracy"],  COLORS["green"],  "✅"), md=True),
            dbc.Col(metric_card("Precision",           km_metrics["precision"], COLORS["cyan"],   "🎯"), md=True),
            dbc.Col(metric_card("Recall",              km_metrics["recall"],    COLORS["purple"], "📡"), md=True),
            dbc.Col(metric_card("F1-Score",            km_metrics["f1"],        COLORS["yellow"], "⚡"), md=True),
            dbc.Col(metric_card("False Positive Rate", km_metrics["fpr"],       COLORS["red"],    "⚠️"), md=True),
        ], className="g-3 mb-3"),

        # PCA Scatter plot
        _section_divider("🔵  K-Means Cluster Visualisation"),
        dbc.Row([
            dbc.Col(
                html.Div(style={
                    "background": COLORS["card_bg"], "borderRadius": "12px",
        ...
        dbc.Row([
            ...
            dbc.Col(
                html.Div(style={
                    "background": COLORS["card_bg"], "borderRadius": "12px",
                    "border": "1px solid rgba(0,212,255,0.12)", "padding": "4px",
            ...
            dbc.Row([
                dbc.Col([
                    *[html.Div(style={
                        ...
                        html.Span(k, style={"color": COLORS["subtext"],
                                            "fontSize": "13px"}),
                        ...
                        ("Total Samples",    f"{total:,}",
                         "white"),
                        ("Normal Traffic",   f"{normal:,} ({normal/total*100:.1f}%)",
                         COLORS["green"]),
                        ("Threats Flagged",  f"{attacks:,} ({attacks/total*100:.1f}%)",
                         COLORS["red"]),
                        ("Row Range",        f"{start_row:,} → {end_row:,}",
                         COLORS["subtext"]),
                    ]],
                ], md=4),
                dbc.Col([
                ...
                dbc.Col([
                    ...
                    dbc.Button("📥 Download Flagged Anomalies (CSV)",
                        id="download-btn", color="success", outline=True,
...
def _footer():
    return html.Div(style={
        ...
        html.Span("🛡️ ANOMCHECKER  ",
            style={"color": COLORS["cyan"], "fontWeight": "700"}),
        html.Span("| Built with Python, Plotly Dash & Scikit-learn | ",
            style={"color": COLORS["subtext"], "fontSize": "13px"}),
        html.Span("Undergraduate Thesis Project",
            style={"color": COLORS["subtext"], "fontSize": "13px"}),
    ])